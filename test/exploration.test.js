...
test("sincroniza preparación, ubicación pública y rechaza zonas u objetos inválidos", () => {
  ...
  assert.throws(() => service.moveDuringExploration(sockets[0], "invented"), (error) => error.code === "INVALID_ZONE");
  assert.throws(() => service.investigateDuringExploration(sockets[0], "missing"), (error) => error.code === "INVALID_OBJECT");
  service.moveDuringExploration(sockets[0], "church");
  assert.equal(service.getRoom(roomCode).players[0].zoneId, "church");
  assert.throws(() => service.investigateDuringExploration(sockets[0], "mud-prints"), (error) => error.code === "OBJECT_NOT_IN_ZONE");
  service.clear();
});
